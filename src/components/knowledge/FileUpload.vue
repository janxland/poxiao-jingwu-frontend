    ...
    <div class="fileupload-container p-0">
        ...
        >
          文件上传
            ...
            <div class="common-doc-section" v-if="currentUploadType === 'json'">
                ...
                <div class="suspect-form">
                    ...
                    <div class="form-item">
                        <label>身份证：</label>
                        <el-input
                            v-model="suspectInfo.cardNumber"
                    ...
                    <div class="form-item file-type-selector">
                        ...
                        <div class="file-type-options">
                            <div
                                ...
                                :class="{
                                    active:
                                        suspectInfo.fileType === fileType.type,
                                }"
                ...
                <div class="common-doc-card">
                    ...
                    <div class="type-name">
                        结构化文件(已支持 TXT、 MARKDOWN、 MDX、 PDF、 HTML、
                        XLSX、 XLS、 DOCX、 CSV、 MD、 HTM，每个文件不超过
                        150MB。)
                    </div>
                    ...
                    >
                        ...
                        <div class="upload-text">
                            ...
                            >
                            <span class="sub-text"
                                >支持上传 JSON、TXT、Excel、Word 文件</span
                            >
                        </div>
                    </el-upload>
                </div>
            ...
            <!-- ======================= 嫌疑人文件上传（Word/Excel） ======================= -->
            <div
                class="suspect-info-section"
                v-if="currentUploadType === 'suspect'"
            >
                <h2>犯罪嫌疑人信息上传</h2>
            ...
            <!-- ======================= 通用公文上传（Word/Excel） ======================= -->
            <div
                class="common-doc-section"
                v-if="currentUploadType === 'common'"
            >
                <h2>法律公文及案例</h2>
            ...
            <div
                ...
                v-if="verificationStatus.completed"
            >
                <div class="verification-header">
                    ...
                    <div
                        ...
                        :class="{
                            'status-passed': verificationStatus.passed,
                            'status-failed': !verificationStatus.passed,
                        }"
                    >
                        <el-icon v-if="verificationStatus.passed"
                            ><Check
                        /></el-icon>
...
function getSelectedFileTypeApiValue() {
    const selectedType = suspectFileTypes.find(
        (item) => item.type === suspectInfo.fileType,
    );
...
function checkWordExcel(file: File) {
    ...
    // 检查文件大小 (限制为10MB)
    const isLt10M = file.size / 1024 / 1024 < 150;
    if (!isLt10M) {
        ElMessage.error('文件大小不能超过10MB!');
        return false;
    }
...
async function startVerification(type: 'json' | 'suspect' | 'common') {
    ...
    // 对嫌疑人上传的场景，要求必填姓名和电话
    if (
        (type === 'suspect' || type === 'json') &&
        (!suspectInfo.name || !suspectInfo.phone)
    ) {
    ...
    filesToVerify.forEach((uploadFile) => {
        tasks.push(
            new Promise((resolve) => {
                ...
                const isLtSize = uploadFile.size / 1024 / 1024 < 150;
                if (!isLtSize) {
                    resultItem.passed = false; // 不支持的文件类型
                    resultItem.issues.push('文件大小不能超过150MB');
                }
                // 判断后缀
                ...
                if (lowerName.endsWith('.json')) {
                    ...
                    resultItem.fileType = 'Markdown';
                } else if (lowerName.endsWith('.mdx')) {
                    resultItem.fileType = 'MDX';
                } else if (lowerName.endsWith('.pdf')) {
                    resultItem.fileType = 'PDF';
                } else if (
                    lowerName.endsWith('.html') ||
                    lowerName.endsWith('.htm')
                ) {
                    resultItem.fileType = 'HTML';
                } else if (
                    lowerName.endsWith('.xlsx') ||
                    lowerName.endsWith('.xls')
                ) {
                    resultItem.fileType = 'Excel';
                } else if (lowerName.endsWith('.csv')) {
                    resultItem.fileType = 'CSV';
                } else {
                    resultItem.passed = false; // 不支持的文件类型
    ...
    // 统计是否全部通过
    verificationStatus.passed = verificationStatus.results.every(
        (r) => r.passed,
    );
    ...
    try {
        ...
        if (failedUploads.length > 0) {
            const failureDetails = failedUploads
                .map((f) => f.reason.fileName)
                .join(', ');
            ElMessage.warning(
                `上传结果: ${successUploads.length}个成功, ${failedUploads.length}个失败`,
            );
        } else {
