          ...
          <div class="info-grid">
            ...
            <div class="info-item">
              ...
              <span class="value">
                <el-tag
                  :type="
                    currentFile.fileType === '开房记录' ? 'success'
                    : currentFile.fileType === '银行流水' ? 'primary'
                    : currentFile.fileType === '聊天记录' ? 'warning'
                    : currentFile.fileType === '交通记录' ? 'danger'
                    : 'info'
                  "
                >
            ...
            <div class="info-item">
              ...
              <span class="value">{{ currentFile.uploadTime }}</span>
            </div>
            <div class="info-item">
              <span class="label">状态：</span>
              <span class="value">
                <el-tag :type="currentFile.status === '已验证' ? 'success' : 'warning'">
                  {{ currentFile.status }}
                </el-tag>
              </span>
            </div>
...
async function viewFileDetails(record) {
  ...
  if (isPreviewableFile(record.fileName)) {
    ...
    try {
      const response = await service.get(`/api/knowledge-base/files/${record.id}/content`);
      if (response.code === 200) {
...
async function downloadFile(file) {
  ...
  try {
    const response = await service.get(`/api/knowledge-base/files/${file.id}/download`, {
      responseType: 'blob'
    });
