...
import { ElSlider, ElInput, ElButton, ElSelect, ElOption, ElForm, ElFormItem, ElTabs, ElTabPane, ElIcon, ElTooltip } from 'element-plus'
...
const allQueryTypes = [
  ...
  // 添加复杂类型字段，但标记为禁用
  { label: '家庭成员犯罪记录', value: 'family_criminal_records', disabled: true, complexType: 'array' },
  { label: '网络信息', value: 'network_info', disabled: true, complexType: 'array' },
]
...
const queryTypes = computed(() => {
  if (!queryTypeSearchKeyword.value) {
    // 如果没有搜索关键词，返回常用字段
    return [
      ...allQueryTypes.filter(item => item.label.includes(queryTypeSearchKeyword.value))
    ]
  }
  
  // 根据关键词过滤
  return allQueryTypes.filter(type => 
...
const emit = defineEmits(['update:queryParams', 'update:timeRange', 'update:riskLevel', 
                          'update:frequency', 'search', 'filter-change'])
...
watch(() => props.queryParams, (newVal) => {
  ...
  // 检查是否有变化
  const hasChanges = Object.keys(newVal).some(key => newVal[key] !== currentParams[key]) ||
                     Object.keys(currentParams).some(key => newVal[key] !== currentParams[key])
          ...
          <div class="query-conditions">
            ...
            >
              <el-select 
                ...
                size="default"
                filterable
                @change="(value) => handleFieldSelect(condition, value)"
              >
                <el-option
    ...
    >
      ...
      <div class="field-selector-grid">
        <el-button
          ...
          :class="[
            'field-selector-item',
            queryTypeSearchKeyword && !(type.label.includes(queryTypeSearchKeyword) || 
            type.value.includes(queryTypeSearchKeyword)) ? 'hidden' : '',
          ...
          @click="() => {
            ...
            const currentCondition = queryConditions.find(c => c.type === 'more' || !c.type);
            if (currentCondition) {
        ...
        >
          {{ type.label }} 
          <span v-if="type.disabled" class="complex-type-tag">({{ type.complexType }})</span>
        </el-button>
      </div>
    </el-dialog>
  </div>
